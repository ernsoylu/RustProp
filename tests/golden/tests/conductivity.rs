...
fn structured_conductivity_matches_upstream() {
    ...
    assert_eq!(fluids.len(), 15, "all structured fluids covered");
    assert!(
        failures.is_empty(),
        "{} of {} failures:\n{}",
        failures.len(),
...
fn conductivity_error_conditions() {
    ...
    // Fully-hardcoded class (Water IAPWS).
    assert!(matches!(
        props_si("L", "T", 300.0, "P", 1e5, "Water").unwrap_err(),
        Error::NotImplemented(_)
    ));
    // Hardcoded critical section (Ammonia).
    assert!(matches!(
        props_si("L", "T", 300.0, "P", 1e6, "Ammonia").unwrap_err(),
        Error::NotImplemented(_)
    ));
    // Structured conductivity whose viscosity class is unported (Benzene's
    // hardcoded higher-order viscosity blocks the OS enhancement).
    assert!(matches!(
        props_si("L", "T", 400.0, "P", 1e6, "Benzene").unwrap_err(),
        Error::NotImplemented(_)
    ));
