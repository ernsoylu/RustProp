...
### 2026-08-21 (second round) — what the fixtures can and cannot say off Linux

The `platform` job's second run failed differently on each host, and both
failures are the same fact wearing two hats: **every oracle record in this repo
was generated by a Linux/x86-64 CoolProp 8.0.0 wheel, so no fixture can say
what CoolProp built against Apple's or MSVC's libm would have refused.** Where
a last-bit property decides refusal-versus-answer, asserting the
classification off Linux asserts something the fixture does not know. The rule
adopted: **keep asserting values everywhere; stop asserting knife-edge
classification off Linux/x86-64**, through a hand-enumerated per-state
allowlist in the affected suite, consulted only under
`!cfg!(all(target_os = "linux", target_arch = "x86_64"))`, with the skip count
ASSERTED (not merely printed) so the set cannot grow unnoticed. A blanket
"skip this suite off Linux" was rejected: most of these records are robust and
their teeth are the whole point of the job.

**Windows-x64, `validity` (3 tests, 16 parity + 33 message failures), all
`PR::Propane` at T=1e30 and T=1e20, P=101325.** The signature was exact powers
of two indexed by derivative order, and the cause is not the EOS at all. At
those temperatures `solve_cubic`'s one-real-root branch returns
`x = t0 - b/(3a)` where both terms are `-713.06352787061...`, while the
physical root `p/(R*T) = 1.2e-26` sits thirteen orders of magnitude BELOW
`ulp(713.06) = 1.1368683772161603e-13`. The density that comes back is
therefore an integer number of ulps of a cancellation — pure rounding residue
— and which integer is decided by the last bits of `sinh`/`asinh`, libm calls
that IEEE-754 does not pin down. Measured: Linux +4 ulp
(`Dmolar = 4.547473508864641e-13`, wheel and port bitwise equal), Windows
+2 ulp (`2.2737367544323206e-13`). Since `rho_reducing = 1` for the cubics,
`delta` IS that density, so `dalpha0_ddelta = 1/delta`,
`d2alpha0_ddelta2 = -1/delta^2` and `d3alpha0_ddelta3 = 2/delta^3` inherit the
factor as exactly 2x, 4x, 8x — which is what the failure output showed. At
T=1e20 the same subtraction gives EXACTLY zero on Linux, so `delta = 0`,
`calc_alpha0_deriv_nocache` returns infinities and upstream's `ValidNumber`
gate throws for 22 of the state's 33 records; Windows lands one ulp away, the
gate never fires, and the port answers. One ulp decides the whole state.

The two `SRK::Propane` states at the same temperatures were **listed too even
though Windows agreed there**, because the same measurement shows the same
mechanism with a two-ulp margin: the root is +2 ulp of
`ulp(-3211.738769906704)` at T=1e30, and -2 ulp at T=1e20 — where the SIGN of
the residue is what produces the "rhomolar is less than zero" refusal that 31
of that state's 33 records assert. Nothing about port fidelity is lost by not
asserting rounding residue; the four states cost 132 of `validity`'s 1,626
records off Linux, and every other record — the HEOS alpha0 gates at T=1e30,
the melting-line refusals, the negative-pressure `post_update` arm, the cubic
states at 300 K and 500 K — keeps asserting on every platform.

**macOS-arm64, `pseudo_pure` (2 parity failures), SES36 `(P, Smolar)` at
P=2846151.** Instrumenting `p_phase_determination_pseudo_pure` on Linux showed
why: the state sits 0.1% under `max_sat_p = 2849000`, where the pseudo-pure PQ
flash's two branches have collapsed. `t_l == t_v == 450.63254754597597` (SES36
gives pL and pV identical coefficients), `rho_l = 0x40a19c5eeb06c55d` and
`rho_v = 0x40a19c5eeb06c56a` are 13 ulp apart, and the two entropies come out
BITWISE EQUAL (`0x40744ada96ae60d6`) — and equal to the requested `Smolar` to
the bit. Upstream's lever rule is then `q = (S - s_l)/(s_v - s_l) = 0/0 = NaN`,
`T = q*t_v + (1-q)*t_l` is NaN, and `post_update` refuses "T is not a valid
number", exactly as the wheel does on the same platform. macOS puts an ulp
between the two entropies, `q` becomes ordinary, and the port answers
`T = 450.63254754597597` / `Dmolar = 2254.185386859511` — which are, to the
digit, the values this fixture's own `(P, Umolar)` records carry at the SAME
pressure. The macOS answer is not a wrong number; it is simply not the wheel's
Linux 0/0, and the fixture cannot arbitrate.

**`--no-fail-fast` on the `platform` job.** `cargo test` stops at the first
failing test BINARY, so run 32468378222 hid every macOS suite after
`pseudo_pure` and every Windows suite after `validity` behind one failure
each — a whole round trip lost to a default. The matrix already sets
`fail-fast: false` for the same reason at the job level; this is that reason
one level down.
